...
- Error 默认开启，其余三项默认关闭。
- 创建项目后生成公开 DSN。
- SDK 指引页展示单包安装命令、DSN 环境变量和 Vue SDK 初始化代码。
- 项目记录连接状态和最后连接时间。
- 没有组的用户登录后进入强制创建组引导。
...
SDK 指引页展示单包 tarball 安装命令、`VITE_PMS_DSN` 环境变量和 `@pms/monitoring-vue` 初始化代码。当前 SDK 初始化只在业务项目内解析并保存 DSN 状态，不采集错误，也不主动发送连接校验请求。
...
- 已验证用户登录后创建组。
- 在组下创建 Vue 项目。
- 在后端接口级使用返回的 DSN 参数调用保留的连接校验端点；该端点不再作为管理端用户接入指引的一部分。
- 所有者查询到已连接状态。
- 其他用户无法访问该组和项目。
